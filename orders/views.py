...
def home(request, username):
    ...
    for order in orders:
        ...
        temp = [item.product for item in items]
        data.append({'order_id': order.id, 'price': order.price, 'items': temp})

    return render(request, 'orders/home.html', {'user': user, 'orders': data})